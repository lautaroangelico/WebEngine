...
function getDownloadsList() {
	...
	$result = $we->query_fetch("SELECT * FROM ".WEBENGINE_DOWNLOADS." ORDER BY download_type ASC, download_id ASC");
...
function addDownload($title, $description='', $link, $size=0, $type=1) {
	...
	$result = $we->query("INSERT INTO ".WEBENGINE_DOWNLOADS." (download_title, download_description, download_link, download_size, download_type) VALUES (?, ?, ?, ?, ?)", array($title, $description, $link, $size, $type));
	if(!$result) return;
...
function editDownload($id, $title, $description='', $link, $size=0, $type=1) {
	...
	$result = $we->query("UPDATE ".WEBENGINE_DOWNLOADS." SET download_title = ?, download_description = ?, download_link = ?, download_size = ?, download_type = ? WHERE download_id = ?", array($title, $description, $link, $size, $type, $id));
	if(!$result) return;
...
function deleteDownload($id) {
	$we = new WebEngineDatabase();
	if(!check_value($id)) return;
	$result = $we->query("DELETE FROM ".WEBENGINE_DOWNLOADS." WHERE download_id = ?", array($id));
	if(!$result) return;
...
function updateDownloadsCache() {
	$we = new WebEngineDatabase();
	$downloadsData = $we->query_fetch("SELECT * FROM ".WEBENGINE_DOWNLOADS." ORDER BY download_type ASC, download_id ASC");
