...
<?php
	...
	$checkBanCron = $dB->query_fetch_single("SELECT * FROM WEBENGINE_CRON WHERE cron_file_run = ?", array("temporal_bans.php"));
	if(!is_array($checkBanCron)) $dB->query($banCron);
	...
	if(check_value($_POST['submit_ban'])) {
		try {
			...
			$logBan = $dB->query("INSERT INTO WEBENGINE_BAN_LOG (account_id, banned_by, ban_type, ban_date, ban_days, ban_reason) VALUES (:acc, :by, :type, :date, :days, :reason)", $banLogData);
			...
			if($banType == "temporal") {
				...
				$tempBan = $dB->query("INSERT INTO WEBENGINE_BANS (account_id, banned_by, ban_date, ban_days, ban_reason) VALUES (:acc, :by, :date, :days, :reason)", $tempBanData);
				if(!$tempBan) throw new Exception("Could not add temporal ban (check tables)[2]. - " . $dB->error);
			}
			
			// Ban Account
			$db = (config('SQL_USE_2_DB',true) ? $dB2 : $dB);
			$banAccount = $db->query("UPDATE "._TBL_MI_." SET "._CLMN_BLOCCODE_." = ? WHERE "._CLMN_USERNM_." = ?", array(1, $_POST['ban_account']));
