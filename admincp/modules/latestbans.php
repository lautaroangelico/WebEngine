...
<?php
	...
	if(check_value($_GET['liftban'])) {
		try {
			...
			$banInfo = $we->query_fetch_single("SELECT * FROM ".WEBENGINE_BAN_LOG." WHERE id = ?", array($_GET['liftban']));
			...
			// Remove Ban log
			$we->query("DELETE FROM ".WEBENGINE_BAN_LOG." WHERE account_id = ?", array($banInfo['account_id']));
			$we->query("DELETE FROM ".WEBENGINE_BANS." WHERE account_id = ?", array($banInfo['account_id']));
	...
	<div class="col-md-12">
		<div class="panel-body">
			...
			<div class="tab-content">
				...
				<?php
					$tBans = $we->query_fetch("SELECT * FROM ".WEBENGINE_BAN_LOG." WHERE ban_type = ? ORDER BY id DESC LIMIT 25", array("temporal"));
					if(is_array($tBans)) {
				...
				<?php
					$pBans = $we->query_fetch("SELECT * FROM ".WEBENGINE_BAN_LOG." WHERE ban_type = ? ORDER BY id DESC LIMIT 25", array("permanent"));
					if(is_array($pBans)) {
						echo '<table class="table table-condensed">';
							echo '<thead>';
							echo '<tr>';
