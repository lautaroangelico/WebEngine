...
$temporalBans = $we->query_fetch("SELECT * FROM ".WEBENGINE_BANS."");
if(is_array($temporalBans)) {
	foreach($temporalBans as $tempBan) {
		...
		if(time() > $banTimestamp) {
			...
			if($unban) {
				$we->query("DELETE FROM ".WEBENGINE_BAN_LOG." WHERE account_id = ?", array($tempBan['account_id']));
				$we->query("DELETE FROM ".WEBENGINE_BANS." WHERE account_id = ?", array($tempBan['account_id']));
			}
		}
	}
}
